...
import com.darwinjacob.springdemo.domain.test.OrganizationRepresentative;
...
public class FormValidationDemoController {
	...
	// format untuk form validation setelah @ModelAttribute harus BindingResult
	...
	if(result.hasErrors()) {
		...
		List<ObjectError> gerrList = result.getGlobalErrors();
		for(ObjectError gerr : gerrList) {
